...
<script type="text/javascript">
	...
	function beforeEditName(treeId, treeNode) {
		$.cookie(window.location.hash.replace("#",""), treeNode.getParentNode().tId);
		$.cuslayer({
			mode:'page',
	...
	function addHoverDom(treeId, treeNode) {
		...
		if (btn) btn.bind("click", function(){
			$.cookie(window.location.hash.replace("#",""), treeNode.tId);
			$.cuslayer({
				mode:'page',
				title:'添加资源',
				height:"${height}",
				width:"${width}",
				url:"${ctxPath!}/"+'${addUrl}',
				data:{"parentId":treeNode.id, "mytid":treeNode.tId}
			});
			return false;
		});
	};
	...
	$(function(){
		...
		var mytid = $.cookie(window.location.hash.replace("#",""));
		if(mytid == "") {
			// 默认展开一级节点
			...
			treeObj.expandNode(treeObj.getNodeByTId(mytid), true, false, false);
		}
