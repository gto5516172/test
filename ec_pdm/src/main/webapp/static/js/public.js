...
function addTab(obj, txt) {
	$.cookie("tabTxt", txt);
	var object = $(obj);
	//tab
	var tab = $("#breadcrumb"),href = object.attr("href");
	var tabTxt = txt;
	var tabli = tab.find("li[h='"+href+"']");
	tab.find("li").removeClass("active");
	if(tabli.size() == 0){ 
		...
		if(mode == 'update'){
			...
			layer.confirm(params.msg,function(index){
				$.ajax({
        			...
        			if(data>0) {
        				if(params.success == undefined){
        					layer.msg('<span class="red bigger-120">更新成功</span>', 1, 1,function(){
            					if(params.reloadurl){
            						location.reload();
            					}else{
            						$curmenu.trigger('click');
        				...
        				layer.alert('<span class="red bigger-120">更新失败，数据正在被使用！</span>', 8, !1);
        			}
        		}).fail(function(error){
        			layer.msg('<span class="red bigger-120">更新失败</span>', 2, 8);
        		});
			},params.title);
			return false;
		}
