...
public abstract class ServiceMybatis<T extends BaseEntity> implements BaseService<T>{
	...
	* @return 未通过返回-1
	...
	public int beforeDeleteTreeStructure(Object id,String Field,Class<?>... beans){
		...
		map.put("id", id);
		map.put("checkField", StringConvert.camelhumpToUnderline(Field).toLowerCase());
		for(int i=0;i<beans.length;i++ ){
			Class<?> cl = beans[i];
			String tableName = StringConvert.camelhumpToUnderline(cl.getSimpleName()).toLowerCase();
