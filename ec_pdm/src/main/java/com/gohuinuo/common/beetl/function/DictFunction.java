...
public class DictFunction{
	...
	* @param type 如sys_data_scope等
	...
	public SysDict getDictListByType(String type,String label){
		List<SysDict> list = (List<SysDict>) sysDictService.findAllMultimap().get(type);
		SysDict sd = null;
		for(int i=0;i<list.size();i++){
			SysDict sdct= list.get(i);
			if(sdct.getLabel().equals(label)){
				sd = sdct;
			}
		}
		return sd;
